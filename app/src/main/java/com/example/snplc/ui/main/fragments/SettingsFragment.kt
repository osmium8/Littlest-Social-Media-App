...
class SettingsFragment : Fragment(R.layout.fragment_settings) {
    ...
    @Inject
    lateinit var glide: RequestManager // load profile image of current user

    private val viewModel: SettingsViewModel by viewModels()

    private var curImageUri: Uri? = null // global reference

    private lateinit var cropContent: ActivityResultLauncher<Any?> // crop an image
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /** get result : cropped image Uri **/
        cropContent = registerForActivityResult(cropActivityResultContract) { uri ->
            uri?.let {
                viewModel.setCurImageUri(it) // will trigger  the observer in this fragment
                // observer assigns value to global current image uri -> @curImageUri
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getUser(uid) // load profile

        btnUpdateProfile.isEnabled = false // button disabled
